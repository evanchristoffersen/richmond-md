...
writeGeomInp() function). Preliminary work on job type 2 has begun. 
...
### JOB TYPE 2 ###
    ...
    print("job2") # python gets mad if there isn't something in this loop
...
# TRYING TO STORE IT ALL IN RAM WONT GO WELL. THIS MEANS AVOID .readlines()

# ADDITIONAL NOTE: The conversion factor for "Atomic Center" values and "ESP
# Fit" values is 0.529177249. Divide by this to convert from Angstroms to Bohr.

    newfile = open('test.txt', 'a')
    with open("GLM1_resp.out", 'r') as f: # opens .out file safely
        for line in f: # search .out file line by line
            if re.search("Atomic Center ", line): # if line contains "Atomic Center "
                newfile.write(line) # write the line to "test.txt"
    with open("GLM1_resp.out", 'r') as f:
        for line in f:
            if re.search("ESP Fit", line): # if line contains "ESP Fit"
                newfile.write(line)
    with open("GLM1_resp.out", 'r') as f:
        for line in f:
            if re.search("Fit    ", line): # if line contains "Fit    "
                newfile.write(line)
    with open("GLM1_resp.out", 'r') as f:
        for line in f:
            if re.search("NGrid ", line): # if line contains "NGrid "
                newfile.write(line)
    newfile.close() # DONT FORGET TO CLOSE FILE NOT OPENED INSIDE A "WITH" STATEMENT

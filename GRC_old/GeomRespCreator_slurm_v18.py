...
def write_resp_input(f,resname,conf,chrg,mult):
...
#     """
#     if type(n) != float:
#         raise ValueError('The value entered must be a float type!\n')
# 
#     if n.startswith('-'):
#         n = n[1:]
#         a = '{:.5E}'.format(n)
...
def write_esp_dat(conf):
    ...
    with open(conf + '_resp.out', 'r') as f, \
        ...
        for i in range(0,len(atomic_centers)):
            if str(atomic_centers[i][0]) == '-0.000000':
                x = '{:>32}'.format('-0.000000E+00')
            else:
                x = numberformat1.write([
                    float(atomic_centers[i][0]) / cnv_factor
                    ])
            if str(atomic_centers[i][1]) == '-0.000000':
                y = '{:>16}'.format('-0.000000E+00')
            else:
                y = numberformat2.write([
                    float(atomic_centers[i][1]) / cnv_factor
                    ])
            if str(atomic_centers[i][2]) == '-0.000000':
                z = '{:>16}'.format('-0.000000E+00')
            else:
                z = numberformat2.write([
                    float(atomic_centers[i][2]) / cnv_factor
                    ])
            esp.write('{}{}{}\n'.format(x, y, z))
    ...
    with open('esp_tmp01.txt', 'r') as b, \
        ...
        for line_b, line_c in zip(b, c):
            ...
            fit = line_c.split()
            if str(fit[2]) == '-0.000000':
                w = '{:>16}'.format('-0.000000E+00')
            else:
                w = numberformat.write([
                    float(fit[2])
                    ])
            if str(esp_fit[6]) == '-0.000000':
                x = '{:>16}'.format('-0.000000E+00')
            else:
                x = numberformat.write([
                    float(esp_fit[6]) / cnv_factor
                    ])
            if str(esp_fit[7]) == '-0.000000':
                y = '{:>16}'.format('-0.000000E+00')
            else:
                y = numberformat.write([
                    float(esp_fit[7]) / cnv_factor
                    ])
            if str(esp_fit[8]) == '-0.000000':
                z = '{:>16}'.format('-0.000000E+00')
            else:
                z = numberformat.write([
                    float(esp_fit[8]) / cnv_factor
                    ])
            # Formats the numbers in fortran format using nonstandard module
            esp.write('{}{}{}{}\n'.format(w, x, y, z))

...
# Module can be imported using the following:
...
def check_filepath(f):
    """
    Raises Exception if a file doesn't exist. Returns None.
    ...
    if os.path.isfile(f) is not True:
        raise Exception('File is missing or does not exist.\n')
    return None
...
def write_esp_dat(conf):
    ...
    # not be overwritten
    if os.path.isfile(conf + '_resp.out') is False:
...
def write_resp_in(f='resp.in',nconfs,ihfree,iqopt,qwt)
    """
    """
    iqopt = '1'
    ihfree = '1'
...
""" JOB TYPE 3 """
    ...
    # preferred over subprocess.call(), subprocess.Popen(), os.system(), and 
    # os.popen().

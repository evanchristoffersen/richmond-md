...
Thank you for the update log. I removed your updates (don't worry they're still
saved in an earlier version) as I reveiwed them. Seriously awesome job. I've
addressed a few of your notes below, and then included a big update on what
I've been up to after that.

Addressing previous notes:

    "I noticed that esp.sh creates a.out, but this script doesn't create it. 
    Nick's guide states this file is important. Something that needs to be 
    considered."

        esp.sh is used in conjunction with a.out (which is an executable
        compiled from readit.f) to create the _esp.dat files and concatenate 
        them into espot. This functionality has been incorporated into this 
        program so we don't need to worry about esp.sh, readit.f, or a.out.
    ...
    not sure d0 needs to be incorporated into the script."
        ...
        I am not sure what d0 means either. I used "diff" to check all 50000 
        lines of numbers in the esp.dat file for GLM1 and GLM2 for Nick's code
        versus this python code when I used unit = 0.529177249 (no d0 at the 
        end) and I got all correct values. My guess is d0 is a fortran number 
        formatting thing and we don't have to worry about it.

    "Something can probably be added to def read_namelist(filename) to extract 
    and set the three letters that name/describe the conformer (e.g. GLD) to a 
    variable that can be used on this line. This is probably a very quick thing
    to do."

        This is already incorporated into the read_namelist() function. See
        these two lines of code just before and just after job type 1 in
        the main program:
            namelist = read_namelist('namelist.in')
            resname = namelist[0]["resname"]
        Here I saved the three letter residue code as "resname"

Evan's Updates for week of 2021.06.11:

I cleaned up the code to abide by PEP8 and PEP257 standards and best practices
since it's starting to get pretty long, and best practices were designed to
make reading and editing code as easy as possible. Major changes include:
    - Enforced line length of no more than 79 characters (you can set whatever
      editor you use to do this automatically)
    - All functions and variables are defined using all lowercase and
      underscores
    - Comments occur on lines before code, not on the same line as the code.
      Additionally, commenting has been reduced to remove clutter - this may be
      a mistake, but I'm curious to hear your opinion.
...
requires, and what the function returns. It also helps us remember what the
different functions are for without having to reinterpret the code.

I improved error messages. Messages are now more concise, and rather than using
sys.exit(), the program is more gracefully exited by Raising an Exception or
built in Error.

Removed your FormatCoordMat() and spacing() functions. The format() command 
built into python accomplishes formatting of columns of data with correct 
spacing without the need to declare any additional functions.

"Commented-out" your fortranFormat() function. A module exists to handle
formatting of scientific notation in fortan's format (first digit is always 
zero followed by a decimal, as opposed to actual scientific format in which the
first digit is the first digit of the actual number followed by a decimal). 
However, this module (called "fortranformat") is not part of the python 
standard library, and while it can still be installed and used on Talapas 
(surprisingly!), it may eventually become necessary to use a self-defined 
function that formats numbers in fortran scientific format. Thus 
fortranFormat() was just commented out but not removed entirely, in case we end
up needing it in the future.

Separated actual program code from function declarations using the standard:
'if __name__="__main__": '

COMPLETED: JOB TYPE 1
    Job type 1 should be ready to go. It needs a test set of files to confirm
    that everything is written properly. This data set would preferably be 
    around 12 conformers total of some small molecule that would take only 
    seconds to minutes to run the calculations for, but would still have at 
    least 12 atoms, where some of the atoms are heavier than Ne. Maybe like a 
    short chain SDS molecule. Let's say propanol with a sulfate head group. I
    chose this molecule because I want to confirm that all of the files format
    correctly when handling numbers that are 1 character and 2 characters long.

IN PROGRESS: JOB TYPE 2 & 3
    Job type 2 and 3 are still under way. Rather than having to choose some 
    random directory to make copies of all of the resp fit files, or assume
    that a student already has access to all of the resp fit files, functions 
    were defined that write most of the files from scratch:

        write_esp_dat() : makes the _esp.dat files and concatenates them into
                          espot 

        write_resp_in() : makes the resp.in files - UNFINISHED
        k
        check_convergence () : checks to see if the resp fitting has finished
                               automatically, still has to be incorporated into
                               a loop

    It is unclear if the leap.in and/or sander.in files are generated by Amber
    or one of the other softwares during this process. If they are not
    automatically generated the following functions can write them from
    scratch:
    ...
    I have no idea where the leaprc.ff02pol.r1 file comes from. It is too long
    to make a function that writes this file. It will have to be copied for
    now, but I think it's really important we figure out where it came from.
    Seems to contain a database of information, and we want to make sure that
    we have the most up-to-date version.

    The popular computational open source tool: "open babel" will automatically
    make .mol2 copies of any and all .xyz files using the command:
        obabel *.xyz -omol2 -m
    but it may require installation of open babel on Talapas, which probably
    isn't possible.

    Additionally, the exact order of operations for steps 8 - 12 in Nick's
    document are not super clear.

        1. What does he mean in step 11 when he says "Go back and repeat steps
           8, 10, and 11 until ... convergence" ? For that matter, are we
           supposed to iterate step 8 over and over until convergence before
           going to step 9, or do we just run the resp command once and then go
           to step 9 and come back later?

        2. Step 9 will likely need to be done by hand outside of this program.
           It seems like you have to evaluate it on a an individual basis, but
           it looks like you just have to modify one file, so no automation is
           needed for this step anyway (probably). What does this mean for the 
           different job types and order of operations in the code? Does job
           type 2 need to be split into two different jobs because of this
           break in the code (steps 8, 10, 11, 12 are all job type 2, but step
           9 needs to be done by hand...)?

        3. How do we determine what parameters do and don't belong in the
           .frcmod file in step 9?

    We are really close to being done. Steps 11 and 12 are basically completed.
    It's just steps 8 - 11 that need clarification.

NOT STARTED: JOB TYPE 4
    Should be easy enough. Just a series of subprocess commands.

Wise Words From Konnor:
Complete job type 2 section. Once completed, run Nick's and the python scripts
on at least two molecules. It seems like GLM makes a good candidate for this
test. Compare the files using the bash "diff" command to insure that Nick's
code has been correctly translated into python. 
...
if __name__=="__main__":
    ...
    if os.path.isfile('namelist.in') is not True:
        raise FileNotFoundError('File namelist.in is missing!\n')
    namelist = read_namelist('namelist.in')
